...
public class EquilibrioClasse
{
    ...
    public void InserirAgendamentoAdm(int id_conflito,int id_motivo,DateTime data_inicio,byte flg_avanco,string texto,int id_mediador,DateTime data_historico,byte tipo_historico,string hora_inicio,string hora_fim,string email_convidado,string link_video)
    {
        try
        {
            MySqlConnection _conn = new MySqlConnection(ConfigurationManager.ConnectionStrings["Mediacao"].ToString());
